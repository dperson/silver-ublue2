...
# Build Image
[group('Image')]
build $image="silver-ublue2" $tag="latest" $flavor="main" rechunk="0" ghcr="0" pipeline="0" $kernel_pin="":
    #!/usr/bin/env -S bash
    ...
    if [[ $(jq "any(.Tags[]; contains(\"$ver\"))" </tmp/repotags.json) == "true" ]]; then
      POINT="1"
      while $(jq -e "any(.Tags[]; contains(\"$ver.$POINT\"))" </tmp/repotags.json); do
        (( POINT++ ))
      done
    fi
    if [[ -n "${POINT:-}" ]]; then
...
[group('Image')]
build-rechunk image="silver-ublue2" tag="latest" flavor="main" kernel_pin="":
    @{{ just }} build {{ image }} {{ tag }} {{ flavor }} 1 0 0 {{ kernel_pin }}


# Build Image with GHCR Flag
[group('Image')]
build-ghcr image="silver-ublue2" tag="latest" flavor="main" kernel_pin="":
    #!/usr/bin/env -S bash
    if [[ "${UID}" -gt "0" ]]; then
...
[group('Image')]
build-pipeline image="silver-ublue2" tag="latest" flavor="main" kernel_pin="":
    #!/usr/bin/env -S bash
...
[group('Image')]
[private]
rechunk $image="silver-ublue2" $tag="latest" $flavor="main" ghcr="0" pipeline="0":
    #!/usr/bin/env -S bash
...
[group('Image')]
load-rechunk image="silver-ublue2" tag="latest" flavor="main":
    #!/usr/bin/env -S bash
    set -eou pipefail
...
# Run Container
[group('Image')]
run $image="silver-ublue2" $tag="latest" $flavor="main":
    #!/usr/bin/env -S bash
    set -euxo pipefail
...
# Build ISO
[group('ISO')]
build-iso $image="silver-ublue2" $tag="latest" $flavor="main" ghcr="0" pipeline="0":
    #!/usr/bin/env -S bash
    set -euxo pipefail
    ...
    if [[ -f "${build_dir}/${image_name}-${tag}-$(uname -m).iso" || \
          -f "${build_dir}/${image_name}-${tag}-$(uname -m).iso-CHECKSUM" ]]; then
    ...
    curl -LSfso build_files/iso/bluefin.repo \
          https://copr.fedorainfracloud.org/coprs/ublue-os/bluefin/repo/fedora-${FEDORA_VERSION}/ublue-os-bluefin-fedora-${FEDORA_VERSION}.repo
...
[group('ISO')]
build-iso-ghcr image="silver-ublue2" tag="latest" flavor="main":
    @{{ just }} build-iso {{ image }} {{ tag }} {{ flavor }} 1


# Run ISO
[group('ISO')]
run-iso $image="silver-ublue2" $tag="latest" $flavor="main":
    #!/usr/bin/env -S bash
    set -eoux pipefail
...
# Secureboot Check
[group('Utility')]
secureboot $image="silver-ublue2" $tag="latest" $flavor="main":
    #!/usr/bin/env -S bash
    set -euo pipefail
...
[group('Utility')]
[private]
fedora_version image="silver-ublue2" tag="latest" flavor="main" $kernel_pin="":
    #!/usr/bin/env -S bash
    set -eou pipefail
    just validate {{ image }} {{ tag }} {{ flavor }}
...
[group('Utility')]
[private]
image_name image="silver-ublue2" tag="latest" flavor="main":
    #!/usr/bin/env -S bash
    set -eou pipefail
    {{ just }} validate {{ image }} {{ tag }} {{ flavor }}
    if [[ "{{ flavor }}" =~ main ]]; then
...
# Generate Tags
[group('Utility')]
generate-build-tags image="silver-ublue2" tag="latest" flavor="main" kernel_pin="" ghcr="0" $version="" github_event="" github_number="":
    #!/usr/bin/env -S bash
    set -euo pipefail
